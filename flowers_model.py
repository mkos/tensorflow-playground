...
# !ls ${PWD}/flowers_trained/export/exporter/

# %bash
# gcloud ml-engine local predict \
#     --model-dir=${PWD}/flowers_trained/export/exporter/1545557890 \
...
def read_and_preprocess(image_bytes, label=None, augment=False):
    ...
    image = None #TODO: decode contents into JPEG
    image = None #TODO: convert JPEG tensor to floats between 0 and 1
    image = tf.expand_dims(image, 0) # resize_bilinear needs batches

    if augment:
       pass #TODO: add image augmentation functions
    else:
...
def serving_input_fn():
    ...
    feature_placeholders = {'image_bytes':
                            tf.placeholder(tf.string, shape=())}
    image = tf.squeeze(feature_placeholders['image_bytes'])
    image, _ = decode_jpeg(image)
    image, _ = resize_image(image)
    image_dict, _ = postprocess_image(image)
    image_dict['image'] = tf.expand_dims(image_dict['image'],0)
    return tf.estimator.export.ServingInputReceiver(image_dict, feature_placeholders)


def decode_jpeg(image_bytes, label=None):
    image = tfi.decode_jpeg(image_bytes, channels=NUM_CHANNELS)
    image = tfi.convert_image_dtype(image, tf.float32) # 0-1
    return image, label


def resize_image(image, label=None):
    image = tf.expand_dims(image, axis=0) # add batch dimension
    image = tfi.resize_bilinear(image, size=[HEIGHT, WIDTH], align_corners=False) # resize
    image = tf.squeeze(image) # remove batch dimension
    return image, label
...
def augment_image(image, label=None):
    image = tf.expand_dims(image, axis=0) # add batch dimension
    image = tfi.resize_bilinear(image, size=[HEIGHT+10, WIDTH+10], align_corners=False) # resize
    image = tf.squeeze(image) # remove batch dimension
...
def postprocess_image(image, label=None):
    # pixel values are in range [0, 1], convert to [-1, 1]
    image = tf.subtract(image, 0.5)
    image = tf.multiply(image, 2.0)
    return {'image': image}, label
