...
plt.style.use(["ggplot", "originals/tensorflowvisu.mplstyle"])
#import matplotlib
...
def tf_format_mnist_images(X, Y, Y_, n=100, lines=10):
    ...
    digits_left = tf.image.grayscale_to_rgb(originals.tensorflowvisu_digits.digits_left())
    correct_tags = tf.gather(digits_left, tf.argmax(Ys_, 1)) # correct digits to be printed on the images
    digits_right = tf.image.grayscale_to_rgb(originals.tensorflowvisu_digits.digits_right())
